...
public class HoopWikipediaFilter extends HoopFilterBase
{    	
	 ...
	 *
	 */
    public HoopWikipediaFilter () 
    {
		setClassName ("HoopWikipediaFilter");
		debug ("HoopWikipediaFilter ()");						
    }  
	/**
	 *
	 */
   ...
   public Boolean cleanBrackets (String aText)   
   {
    ...
    public String clean (String aText)   
    {
    	...
    	for (int i=0;i<HoopLink.garbage.length;i++)
    	{
    		again=true;
    	
    		while (again==true)
    		{
    			again=cleanCharacter (manipulator,HoopLink.garbage [i]);
    		}
    	}	
    ...
    public Boolean check (String aText)   
    {
    	...
    	if (HoopStringTools.isInteger(manipulator)==true)
    	{
	...
	{
    	HoopWikipediaFilter filter=new HoopWikipediaFilter ();

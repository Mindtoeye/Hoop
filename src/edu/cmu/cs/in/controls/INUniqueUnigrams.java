...
public class INUniqueUnigrams extends JPanel implements ActionListener
{		
	private INDataCollection INData=null;	
	private INFileManager fManager=null;
	
	public INJFeatureList posList=null;
    public INJFeatureList negList=null;
    			...
    			{    	    	 
    				...
    				for (int g = 0; g < attributes.getLength(); g++) 
    				{
    					...
    					if (attribute.getName ().equals ("name")==true)
    					{
    						...
    						INVisualFeature testFeature=new INVisualFeature ();
    						...
    						for (int j=0;j<featList.getLength();j++)
    						{
    							...
    							for (int t=0;t<INData.classes.size();t++)
    							{        
    								INVisualClass cl=(INVisualClass) INData.classes.get(t);
                    	        	                    	        	        		
    								HashMap<String,INVisualFeature>feats=cl.getFeatures();
  
    								INVisualFeature checkFeature=feats.get(testFeature.text);
    ...
    private void toXML ()
    {
        ...
        for (int j=0;j<INData.classes.size();j++)
        {        
        	INVisualClass cl=(INVisualClass) INData.classes.get(j);
        	
        	loader.append("\t<class name=\""+cl.getName()+"\">\n");
        	        		
        	HashMap<String,INVisualFeature>feats=cl.getFeatures();
        	Collection <INVisualFeature>col=feats.values();
        	Iterator<INVisualFeature>itr = col.iterator();
        		        		
       		while(itr.hasNext())
       		{        
       			INVisualFeature checker=itr.next();
